...
/**
 * Updates data about all ConfBridge conferences.
 *
 * @method updateConfBridgeConferences
 * @param {object} err  The error response object
 * @param {object} data The ConfBridge conferences information object
 * @private
 */
function updateConfBridgeConferences(err, data) {
  try {
    if (err) {
      logger.error(IDLOG, 'updating data about all ConfBridge conferences: ' + err.toString());
      return;
    }
    if (typeof data === 'object') {
      ...
      for (extOwnerId in data) {
        updateConfBridgeConf(null, data[extOwnerId]);
      }
    }
    logger.info(IDLOG, 'updated all ConfBridge conferences: #' + Object.keys(conferences).length);
...
/**
 * Updates data about a single ConfBridge conference.
 *
 * @method updateConfBridgeConf
 * @param {object} err  The error response object
 * @param {object} data The ConfBridge conference information object
 * @private
 */
function updateConfBridgeConf(err, data) {
  try {
    if (err) {
      logger.error(IDLOG, 'updating data about single ConfBridge conference: ' + err.toString());
      return;
    }

    // data can be undefined when no conference is present. This case is
    // managed by "confbridgeend" event
    if (typeof data === 'object') {

      var i;
      var newConf = new ConfBridgeConference(data.confId);
      for (i = 0; i < data.users.length; i++) {
        var newUserConf = new ConfBridgeConfUser(data.users[i].id, data.users[i].extenId, data.users[i].isOwner, data.users[i].channel);
      ...
      logger.info(IDLOG, 'updated ConfBridge conference "' + data.confId + '": users #' + data.users.length);

      // emit the event
      logger.info(IDLOG, 'emit event ' + EVT_CONFBRIDGE_CONF_CHANGED + ' for conf ' + data.confId);
      astProxy.emit(EVT_CONFBRIDGE_CONF_CHANGED, conferences[data.confId]);
    }
...
/**
 * Initializes all ConfBridge conferences.
 *
 * @method initConfBridgeConf
 * @private
 */
function initConfBridgeConf() {
  try {
    astProxy.doCmd({
      command: 'listConfBridgeConf',
      confBridgeConfCode: getConfBridgeConfCode(),
      remoteSitesPrefixes: remoteSitesPrefixes
    }, updateConfBridgeConferences);
 ...
 * @private
 */
function updateExtenConversations(err, resp, exten, spyConnected) {
  try {
    if (err) {
    ...
    if (extensions[exten]) {
      ...
      for (chid in resp) {
        ...
        if (chid.indexOf('Local') === -1 &&
          ...
          addConversationToExten(ext, resp, chid, spyConnected);
        }
      }
      // emit the event
 ...
 * @private
 */
function addConversationToExten(exten, resp, chid, spyConnected) {
  try {
    // check parameters
      ...
      for (ch2 in resp) {
        ...
        if (
          ...
          (
            ...
            (resp[ch].status === 'ring' || resp[ch].status === 'ringing' || resp[ch].status === 'up') &&
            (resp[ch2].status === 'ring' || resp[ch2].status === 'ringing' || resp[ch2].status === 'up') &&
            resp[ch2].channel !== resp[ch].channel
          )
        ) {
    ...
    if ((extensions[exten] || tempExtensions[exten]) && resp[chid].bridgedChannel !== undefined) {
      ...
      let conv = new Conversation(exten, chSource, chDest, queue, resp[chid].linkedid, resp[chid].uniqueid, spyConnected);
...
function sipTrunkStatus(err, resp) {
  try {
    ...
    if (resp.trunk && trunks[resp.trunk] && resp.status) {
 ...
 */
 function trunkRegistration(err, resp) {
  try {
    ...
    if (resp.trunk && resp.registration && trunks[resp.trunk] ) {
 ...
 * @return {object} The _ConfBridgeConference_ object.
 */
...
/**
 * Updates the peer registration.
 * To update the trunks list it request all trunks
 * to analize through "Registry" command plugin.
 *
 * @method evtRegistryChanged
 * @param {object} peer The peer data
 * @param {string} peerStatus The adapted status
 * @private
 */
 function evtRegistryChanged(peer, peerStatus) {
  try {
    if (typeof peer !== 'object' || typeof peerStatus !== 'string') {
      throw new Error('wrong parameters: ' + JSON.stringify(arguments));
    }

    for (let trunk in staticDataTrunks) {
      if (trunks[trunk] &&
        peer.domain.includes(staticDataTrunks[trunk].host) &&
        peer.username.includes(staticDataTrunks[trunk].username)) {
        // update trunk registration
        trunks[trunk].setRegistration(peerStatus);
      }
    }
  } catch (err) {
    logger.error(IDLOG, err.stack);
...
/**
 * A user extension has left the ConfBridge conference. So update info about the conference.
 *
 * @method evtRemoveConfBridgeUserConf
 * @param {object} data The response object received from the event plugin _confbridgeleave_.
 */
function evtRemoveConfBridgeUserConf(data) {
  try {
    // check parameter
    if (typeof data !== 'object' ||
    ...
    logger.info(IDLOG, 'user id "' + data.userId + '" with exten id "' + data.extenId + '" has left the ConfBridge conf ' + data.confId);
    setTimeout(function() {
      astProxy.doCmd({
        command: 'listConfBridgeConf',
        confBridgeConfCode: getConfBridgeConfCode(),
        ...
        updateConfBridgeConf(err, resp[(Object.keys(resp))[0]]);
      });
    },250)
  } catch (err) {
    logger.error(IDLOG, err.stack);
...
/**
 * A ConfBridge conference has been ended. So update info about the conference.
 *
 * @method evtRemoveConfBridgeConf
 * @param {object} data The response object received from the event plugin _confbridgeend_.
 */
function evtRemoveConfBridgeConf(data) {
  try {
    // check parameter
    if (typeof data !== 'object' ||
      typeof data.confId !== 'string') {
    ...
    logger.info(IDLOG, 'ConfBridge conf "' + data.confId + '" has been ended');
    delete conferences[data.confId];

    // emit the event
    logger.info(IDLOG, 'emit event ' + EVT_CONFBRIDGE_CONF_END + ' for conf ' + data.confId);
    astProxy.emit(EVT_CONFBRIDGE_CONF_END, data.confId);
...
/**
 * A user extension has joined the ConfBridge conference. So update info about the conference.
 *
 * @method evtAddConfBridgeUserConf
 * @param {object} data           The response object received from the event plugin _confbridgejoin_.
 ...
 */
function evtAddConfBridgeUserConf(data) {
  try {
    // check parameter
    if (typeof data !== 'object' ||
    ...
    logger.info(IDLOG, 'user id "' + data.userId + '" with exten id "' + data.extenId + '" has joined the ConfBridge conf ' + data.confId);
    setTimeout(function() {
      astProxy.doCmd({
        command: 'listConfBridgeConf',
        confBridgeConfCode: getConfBridgeConfCode(),
...
/**
 * The mute status of a ConfBridge user conference has been changed. So update info about the conference.
 *
 * @method evtConfBridgeUserConfMute
 * @param {object} data The response object received from the event plugin _confbridgemute_.
 */
function evtConfBridgeUserConfMute(data) {
  try {
    // check parameter
    if (typeof data !== 'object' ||
    ...
    logger.info(IDLOG, 'mute status of user id "' + data.userId + '" of ConfBridge conf "' + data.confId + '" has been changed to ' + data.mute);
    setTimeout(function() {
      astProxy.doCmd({
        command: 'listConfBridgeConf',
        confBridgeConfCode: getConfBridgeConfCode(),
...
function evtSpyStartConversation(data) {
  try {
    ...
    astProxy.doCmd({
      ...
      // set the spy conversation as connected
      const spyConnected = true;
      // update the conversations of the spier
      if (extensions[data.spierId]) {
        updateExtenConversations(err, resp, data.spierId, spyConnected);
      }
    });
  ...
  try {
    ...
    if (extensions[data.chDestExten]) {
      ...
      astProxy.emit(EVT_EXTEN_DIALING, {
        ...
        destUniqueId: data.destUniqueId || ''
      });
    }
...
/**
 * Mute a user of a ConfBridge conference.
 *
 * @method muteUserConfBridgeConf
 * @param {string} confId The ConfBridge conference identifier
 ...
 * @param {function} cb The callback function
 */
function muteUserConfBridgeConf(confId, userId, extenId, direction, cb) {
  try {
    // check parameters
    if (typeof cb !== 'function' ||
    ...
    astProxy.doCmd({
      ...
      function (error, resp) {
        ...
        if (resp[userId] && resp[userId].channel) {
          logger.info(IDLOG, 'execute mute of user "' + userId + '" (ch: ' + resp[userId].channel + ') of ConfBridge conf "' + confId + '" on direction "' + direction + '"');
          astProxy.doCmd({
              command: 'mute',
            ...
            function (error) {
              cb(error);
              muteUserConfBridgeConfCb(error);
            });
        } else {
          var str = 'ConfBridge channel to mute not found for userId "' + userId + '" (exten: ' + extenId + ') for confId "' + confId + '"';
...
/**
 * Unmute a user of a ConfBridge conference.
 *
 * @method unmuteUserConfBridgeConf
 * @param {string} confId The ConfBridge conference identifier
 * @param {string} userId The user indentifier into the conference
 * @param {string} extenId The extension indentifier of the user
 ...
 * @param {function} cb The callback function
 */
function unmuteUserConfBridgeConf(confId, userId, extenId, onlyListen, cb) {
  try {
    // check parameters
    if (typeof cb !== 'function' ||
      typeof confId !== 'string' ||
    ...
    astProxy.doCmd({
      ...
      function (error, resp) {
        ...
        if (resp[userId] && resp[userId].channel) {
          logger.info(IDLOG, 'execute unmute of user "' + userId + '" (ch: ' + resp[userId].channel + ') of ConfBridge conf "' + confId + '" on direction "' + direction + '"');
          astProxy.doCmd({
              command: 'unmute',
              channel: resp[userId].channel,
              direction: direction
          ...
          var str = 'ConfBridge channel to unmute not found for userId "' + userId + '" (exten: ' + extenId + ') for confId "' + confId + '"';
          logger.error(IDLOG, str);
          cb(str);
        }
      });
...
/**
 * Hangup a user of a ConfBridge conference.
 *
 * @method hangupUserConfBridgeConf
 * @param {string}   confId  The ConfBridge conference identifier
 * @param {string}   extenId The extension indentifier into the conference
 * @param {function} cb      The callback function
 */
function hangupUserConfBridgeConf(confId, extenId, cb) {
  try {
    // check parameters
    if (typeof cb !== 'function' ||
      typeof confId !== 'string' ||
    ...
    logger.info(IDLOG, 'execute hangup of user "' + extenId + '" of ConfBridge conf "' + confId + '"');
    var warn;
...
function pickupParking(parking, destId, extForCtx, cb) {
  try {
    ...
    // check if the dest extension exists
    if (extensions[destId]) {

      // ask the info of the parked calls at runtime
      astProxy.doCmd({
        command: 'listParkedCalls'
      }, (err, list) => {

        // check action error
        if (err) throw new Error('error listing parked calls')

        // check parked calls length
        if (!(Object.keys(list).length > 0)) throw new Error('no parked calls found')

        const ctx = extensions[extForCtx].getContext();
        const ch = list[parking].channel;
        ...
        if (ch !== undefined) {
          ...
          // pickup the call using the redirectChannel action
          astProxy.doCmd({
...
/**
 * Starts a ConfBridge conference. Its behaviour change based on owner
 * extension status of the conference.
 *
 * @method startConfBridgeConference
 ...
 * @param {function} cb           The callback function
 */
function startConfBridgeConference(convid, ownerExtenId, addExtenId, cb) {
  try {
    // check parameters
    ...
    if (!extensions[ownerExtenId]) {
      warn = 'starting ConfBridge conf of owner exten "' + ownerExtenId + '" failed: exten non existent';
    ...
    if (!conv) {
      warn = 'starting ConfBridge conf of owner exten "' + ownerExtenId + '" failed: convid "' + convid + '" non existent';
      logger.warn(IDLOG, warn);
      cb(warn);
      return;
    }
    ...
    var confnum = getConfBridgeConfCode() + ownerExtenId;
...
/**
 * Ends the entire ConfBridge conference.
 *
 * @method endConfBridgeConf
 * @param {string}   confId The conference identifier
 * @param {function} cb     The callback function
 */
function endConfBridgeConf(confId, cb) {
  try {
    // check parameters
    ...
    if (!conferences[confId]) {
      var warn = 'ending entire ConfBridge conf "' + confId + '" failed: conf does not exist';
      logger.warn(IDLOG, warn);
      cb(warn);
      return;
    }
    ...
    logger.info(IDLOG, 'starting end entire ConfBridge conf "' + confId + '"');
    async.each(arrUsers, function (u, eachCb) {
      ...
      if (err) {
        logger.error(IDLOG, 'ending entire ConfBridge conf "' + confId + '": ' + err.toString());
        cb(err);
      } else {
        logger.info(IDLOG, 'entire ConfBridge conf "' + confId + '" has been ended successfully');
        cb(null);
      }
    });
...
/**
 * Returns true if the extension is already into its ConfBridge conference.
 *
 * @method isExtenInConfBridgeConf
 * @param  {string}  ownerExtenId The extension owner of the conference
 * @return {boolean} True if the extension is already into its ConfBridge conference.
 */
function isExtenInConfBridgeConf(ownerExtenId) {
  try {
    // check parameters
    if (typeof ownerExtenId !== 'string') {
      throw new Error('wrong parameters: ' + JSON.stringify(arguments));
...
/**
 * This is the callback of the mute user of a ConfBridge conference command plugin.
 *
 * @method muteUserConfBridgeConfCb
 * @param {object} error The error object of the operation
 * @private
 */
function muteUserConfBridgeConfCb(error) {
  try {
    if (error) {
      logger.warn(IDLOG, 'mute user of ConfBridge conf failed: ' + error.message);
    } else {
      logger.info(IDLOG, 'mute user of ConfBridge conf succesfully');
    }
  } catch (err) {
    logger.error(IDLOG, err.stack);
  }
...
/**
 * This is the callback of the unmute user of a ConfBridge conference command plugin.
 *
 * @method unmuteUserConfBridgeConfCb
 * @param {object} error The error object of the operation
 * @private
 */
function unmuteUserConfBridgeConfCb(error) {
  try {
    if (error) {
      logger.warn(IDLOG, 'unmute user of ConfBridge conf failed: ' + error.message);
    } else {
      logger.info(IDLOG, 'unmute user of ConfBridge conf succesfully');
    }
...
/**
 * Books a recall to a busy extension.
 *
 * @method recallOnBusy
 * @param {string}  caller  The main extension of the caller
 * @param {string}  called  The main extension of the called
 * @param {function}  cb    The callback function
 * @returns {array} The waiting extensions
 */
 function recallOnBusy(caller, called, cb) {
  try {
    // check parameters
    if (typeof caller !== 'string' || typeof called !== 'string') {
      throw new Error('wrong parameters: ' + JSON.stringify(arguments));
    }
    // exec command to check the already waiting extensions
    astProxy.doCmd({
      command: 'robGet',
      called: called
    }, function (err, resp) {
      ...
      let extensionsWaiting = resp.extensions_waiting ? resp.extensions_waiting.split('&') : []
      // add the caller to the waiting extensions list if not already exists
      if (extensionsWaiting.indexOf(caller) === -1) extensionsWaiting.push(caller)

      // update waiting extensions list
      astProxy.doCmd({
        command: 'robSet',
        called: called,
        extensions_waiting: extensionsWaiting.join('&')
      }, function (err) {
        cb(err, {
          extensions_waiting: extensionsWaiting
        })
      })
    });
  } catch (err) {
...
exports.recallOnBusy = recallOnBusy;
...
exports.endConfBridgeConf = endConfBridgeConf;
...
exports.getConfBridgeConfCode = getConfBridgeConfCode;
exports.getJSONExtensions = getJSONExtensions;
exports.queueMemberRemove = queueMemberRemove;
exports.EXTEN_STATUS_ENUM = EXTEN_STATUS_ENUM;
...
exports.muteUserConfBridgeConf = muteUserConfBridgeConf;
...
exports.isExtenInConfBridgeConf = isExtenInConfBridgeConf;
exports.evtRemoveConfBridgeConf = evtRemoveConfBridgeConf;
exports.evtQueueMemberAdded = evtQueueMemberAdded;
exports.EVT_CONFBRIDGE_CONF_END = EVT_CONFBRIDGE_CONF_END;
...
exports.unmuteUserConfBridgeConf = unmuteUserConfBridgeConf;
exports.hangupUserConfBridgeConf = hangupUserConfBridgeConf;
exports.evtAddConfBridgeUserConf = evtAddConfBridgeUserConf;
...
exports.evtConfBridgeUserConfMute = evtConfBridgeUserConfMute;
...
exports.startConfBridgeConference = startConfBridgeConference;
exports.evtExtenStatusChanged = evtExtenStatusChanged;
exports.evtRegistryChanged = evtRegistryChanged;
...
exports.evtRemoveConfBridgeUserConf = evtRemoveConfBridgeUserConf;
...
exports.EVT_CONFBRIDGE_CONF_CHANGED = EVT_CONFBRIDGE_CONF_CHANGED;
