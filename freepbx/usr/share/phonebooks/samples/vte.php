...
if (!empty($data['data'][0]['count'])) {
	...
	for ($offset = 0; $offset < $count; $offset+=$limit) {
		...
		if (!empty($data['data']) && is_array($data['data'])) {
			$query_insert = 'INSERT INTO phonebook (name,company,title,workphone,cellphone,homephone,workemail,fax,workstreet,workpob,workcity,workprovince,workpostalcode,workcountry,firstname,lastname,job,facebook,instagram,linkedin,workphone2,cellphone2,otherphone,otheremail,type,access,sid_imported) VALUES ';
			$questionmarks = [];
			$query_data = [];
			foreach ($data['data'] as $record) {
				...
				$query_data[] = 'vte';    // type
				$query_data[] = 'public'; // access
				$query_data[] = 'vte';    // sid_imported
			}
