...
- `LOGIN_LOGO_URL` Public URL for the login logo shown inside the wizard sign-in card
- `FAVICON_URL` Public URL for the browser favicon used by the wizard
- `LOGIN_BACKGROUND_URL` Public URL for the wizard login background image
- `LOGIN_PEOPLE` Toggle the right-side wizard illustration (`show` or `hide`)
...
The wizard login page loads `scripts/custom.js` at runtime. That file is generated by `entrypoint.sh` inside the FreePBX container and contains the values for `LOGIN_LOGO_URL`, `FAVICON_URL`, `LOGIN_BACKGROUND_URL` and `LOGIN_PEOPLE`.

Because of this, changing only the frontend build is not enough to validate login branding. If `scripts/custom.js` is unchanged, the wizard keeps the previous runtime logo, favicon, background and illustration visibility even after a static deploy.

Quick runtime check inside the container:

```bash
podman exec freepbx sh -lc "sed -n '1,20p' /var/www/html/freepbx/wizard/scripts/custom.js"
```
