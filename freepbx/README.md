...
## Satellite TTS (satellite module)
...
- `http://127.0.0.1:${SATELLITE_HTTP_PORT}/api/get_speech`

### Class methods (`admin/modules/satellite/Satellite.class.php`)

- `Satellite::get_available_voices()`
    - Fetches available TTS models from `http://127.0.0.1:${SATELLITE_HTTP_PORT}/api/get_models`.
    - Returns an associative array keyed by language code, each containing a sorted list of model names.

- `Satellite::tts($text, $model = '', $language = 'en', $force = false)`
    - Validates input (`text` is required).
    - Calculates checksum `md5(text|model|language)`.
    - Reuses existing `/tmp/satellite-<checksum>.mp3` when available and `$force` is `false`.
    - Calls Satellite API and stores the returned MPEG audio in `/tmp`.
    - Returns checksum on success.

- `Satellite::get_unsaved_audio($checksum)`
    - Validates checksum format (32-char hex string).
    - Returns base64-encoded temporary MP3 content for preview/download.
...
- `satellite_get_config($engine)` — early config hook (currently unused).
- `satellite_get_config_late($engine)` — when `SATELLITE_CALL_TRANSCRIPTION_ENABLED` is `True`, injects MixMonitor and Stasis dialplan entries for real-time call transcription into `macro-dial-one` and `macro-dialout-trunk`.

### Example calls

Generate TTS and get checksum:

```php
$checksum = FreePBX::Satellite()->tts('Ciao, questo è un messaggio di prova', 'aura-2-melia-it');
```
or
```php
$checksum = FreePBX::Satellite()->tts('Hello, this is a test message', '', 'en');
```

Read temporary audio (base64):

```php
$audioBase64 = FreePBX::Satellite()->get_unsaved_audio($checksum);
```

Save temporary audio as recording:

```php
$recordingId = FreePBX::Satellite()->save_recording($checksum, 'en', 'Test TTS', 'Generated by Satellite TTS');
```

Save recording directly from text:

```php
$recordingId = FreePBX::Satellite()->save_recording('', 'en', 'Welcome message', 'Generated from text', 'Welcome to our service');
```

Get available TTS voices grouped by language:

```php
$voices = FreePBX::Satellite()->get_available_voices();
```

Delete a cached temporary MP3 when it is no longer needed:

```php
FreePBX::Satellite()->delete_temp_audio($checksum);
```
