...
## Satellite TTS (nethcti3 module)

The `nethcti3` module provides helper functions to generate TTS audio through
Satellite, preview the generated audio, and save it as a FreePBX recording.
...
- `Nethcti3SatelliteTts::tts($text, $model = '', $language = 'en', $force = false)`
    - Validates input (`text` is required).
    - Calculates checksum `md5(text|model|language)`.
    - Reuses existing `/tmp/<checksum>.mp3` when available and `$force` is `false`.
    - Calls Satellite API and stores the returned MPEG audio in `/tmp`.
    - Returns checksum on success.

- `Nethcti3SatelliteTts::get_unsaved_audio($checksum)`
    - Returns base64-encoded temporary MP3 content for preview/download.

- `Nethcti3SatelliteTts::save_recording($filename = '', $language = 'en', $name = '', $description = '', $text = '', $model = '')`
...
```php
$checksum = nethcti3_tts('Ciao, questo è un messaggio di prova', 'aura-2-melia-it');
```
or
```php
...
```php
$audioBase64 = nethcti3_get_unsaved_audio($checksum);
```

Save temporary audio as recording:

```php
$recordingId = nethcti3_save_recording($checksum, 'en', 'Test TTS', 'Generated by Satellite TTS');
```

Save recording directly from text (class method):

```php
$tts = new Nethcti3SatelliteTts();
$recordingId = $tts->save_recording('', 'en', 'Welcome message', 'Generated from text', 'Welcome to our service');
```
