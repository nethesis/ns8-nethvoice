...
The wizard login page loads `scripts/custom.js` at runtime. That file is generated by `entrypoint.sh` inside the FreePBX container and contains the values for `LOGIN_LOGO_URL`, `FAVICON_URL` and `LOGIN_BACKGROUND_URL`.

Because of this, changing only the frontend build is not enough to validate login branding. If `scripts/custom.js` is unchanged, the wizard keeps the previous runtime logo, favicon and background even after a static deploy.
