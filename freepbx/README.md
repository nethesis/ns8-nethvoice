...
- `SMTP_FROM_ADDRESS` from address for voicemail emails "nethvoice@example.com". If not set, -f option can be used to set the from address from the voicemail command from FreePBX. Otherwise it will be generated from `BRAND_NAME` and `SMTP_USERNAME` (if it contains a @ and ) or `SMTP_HOST`.
