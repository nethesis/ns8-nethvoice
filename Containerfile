FROM docker.io/library/node:24.11.1-slim as ui_builder
WORKDIR /app
# install deps
...
RUN --mount=type=cache,id=nethvoice-module-ui-yarn,target=/var/cache/yarn \
    corepack enable && \
    YARN_CACHE_FOLDER=/var/cache/yarn yarn install --immutable
# copy application
...
ENTRYPOINT [ "/" ]
LABEL org.nethserver.authorizations="traefik@any:fulladm node:fwadm,portsadm nethvoice-proxy@any:routeadm nethvoice-proxy@any:trunkadm cluster:accountconsumer openldap@any:domadm samba@any:domadm"
