...
class ModuleRemovedEventTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.agent = types.ModuleType("agent")
        cls.agent.SD_DEBUG = ""
        cls.handler = load_script(
            "module_removed_users_admin",
            REPOSITORY_ROOT
            / "imageroot"
            / "events"
            / "module-removed"
    ...
    def test_reconfigures_users_admin_after_cluster_module_removal(self):
    ...
    def test_ignores_module_removal_from_invalid_source(self):
        with mock.patch.dict(
            os.environ,
            {
