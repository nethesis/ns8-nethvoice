...
container=$(buildah from ghcr.io/nethesis/nethcti-middleware:satellite-mqtt)
...
container=$(buildah from ghcr.io/nethesis/nethvoice-cti:phoneIslandTrascription)
...
container=$(buildah from ghcr.io/nethesis/satellite:fix_speaker_swap)
# Commit the image
buildah commit "${container}" "${repobase}/${reponame}"
buildah commit "${container}" "${repobase}/${reponame}:${IMAGETAG:-latest}"
# Append the image URL to the images array
